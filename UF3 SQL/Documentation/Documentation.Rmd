...
En el caso del SELECT INTO, solo se puede devolver un resultado en el select (una fila) ya
que sino no podríamos guardarlo en una variable.
...
-Ruta del fichero: Debe ser absoluta o relativa a partir de la carpeta donde esta instalado el
...
En el siguiente ejemplo copiamos lo que hay en el fichero y lo introducimos en la BBDD pero ignorando la 1ª linea:
...
-Ruta del fichero: Debe ser absoluta o relativa a partir de la carpeta donde esta instalado elSGBD. La ruta al fichero se tiene que formar con /
